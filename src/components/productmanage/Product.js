...
const Product = ({ setTotalPages, page, setswitsh, setitem }) => {
      ...
      try {
        const products = async () => {
          const data = await fetch(
            `${baseUrl()}/products/edit/${product._id}/`,
            {
              method: "DELETE",
              headers: {
          ...
          if (data.status === 204) window.location.reload(true);
            ...
            setresponse({
              type: "error",
              message:
                "failed to delete  product , please login and try again!!",
              isExist: true,
              action: "login",
            });
        };
        ...
        setresponse({
          type: "error",
          message: "failed to delete  product , please login and try again!!",
          isExist: true,
          action: "login",
        });
      }
  ...
  useEffect(() => {
    try {
      const products = async () => {
        const data = await fetch(`${baseUrl()}/products/${page}`, {
          method: "GET",
      ...
      setresponse({
        type: "error",
        message: "failed to load data , please login and try again!!",
        isExist: true,
        action: "login",
      });
    }
          ...
          <div className="manage-card" key={index}>
            ...
            <div className="manage-details">
              ...
              <div className="manage-reflexionSection">
                ...
                <div className="manage-homeprice">
                  <p> {el.price} $ </p>
                </div>
              </div>
            </div>
