    ...
    (product) => async () => {
      try {
        const products = async () => {
          const data = await fetch(`${baseUrl()}/orders/${product._id}/`, {
            method: "PUT",
            headers: {
          ...
          if (data.status === 200) window.location.reload(true);
            ...
            setresponse({
              type: "error",
              message:
                "failed to archive order , please login and  try again!!",
              isExist: true,
              action: "login",
            });
        };
        ...
        setresponse({
          type: "error",
          message: "failed to archive order , please login and  try again!!",
          isExist: true,
          action: "login",
        });
      }
      ...
      try {
        const products = async () => {
          ...
          if (data.status === 204) window.location.reload(true);
            ...
            setresponse({
              type: "error",
              message: "failed to delete order , please  login and try again!!",
              isExist: true,
              action: "login",
            });
        };
    ...
    try {
      const products = async () => {
        ...
        if (data.status === 200) {
          ...
          setresponse({
            type: "error",
            message: "faild to load data , please login",
            isExist: true,
            action: "login",
          });
        }
      ...
      {allorders > 0 && (
        <div className="product_container">
          <div className="order_title">
            {" "}
            <p>your orders {allorders > 0 && <span>{allorders}</span>}</p>
          </div>
          {response.isExist && (
            <Alert setresponse={setresponse} response={response} />
          )}
            ...
            return (
              <div className="product_order" key={index}>
                ...
                <div className="data_important">
                  <div className="customer_totals">{el.total} $</div>
                  <div className="customer_button">
                    <button onClick={handlchek(el)}>
                      <FontAwesomeIcon icon={faCheck} />
                    </button>
                    <button onClick={handldelete(el)}>
